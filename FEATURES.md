...
- Dynamische Zusatzfelder pro Entity-Typ: Definition von Feldern (Text, Zahl, Select, Boolean) im UI (Type-Editor), die automatisch in den Entity-Formularen gerendert und validiert werden; die Detailansicht zeigt alle definierten Felder inkl. leeren Platzhaltern an.
- Custom Fields lassen sich direkt in der Entity-Detailansicht bearbeiten; die UI übernimmt Typen (Boolean, Number, Select, Text) und bietet optional eine Roh-JSON-Eingabe für Spezialfälle.
...
- Persistente Vor-Klassifizierung: `inventory_classifications` speichert Achsenwerte, bevor ein Asset/Revision entsteht. `entity_files.php` filtert offene Dateien nach diesen Werten, zeigt sie in der Liste an und übernimmt sie beim finalen Speichern in `asset_classifications`/`revision_classifications`.
...
- SUBJECT_FIRST-Vision-Prepass mit strengem JSON-Schema (Subjects, Counts, human_attributes, image_kind, background_type, Notes, Caption, Confidence), Speicherung in `asset_ai_prepass` inkl. Modell, Revision, Confidence; Soft-Priors (character/location/scene/prop/effect) via `PrepassScoringService`, Audit-Diffs. Endpoint `POST /api/v1/ai/prepass-subject`, CLI `php bin/ai-prepass-subject.php --asset=<id>`, UI-Panel & Button im Asset-Detail.
- Pipeline: lokales Bild laden → neutrale VISUAL-EXTRACTION (Motiv-Art, living_kind, gender/age-Hints, Keywords, Setting, Stil, Caption + Confidence) → Entity-Type-Selektion mit dynamischen `entity_types`/`field_definitions` + Prepass-Priors → Entity-Ranking über Metadata-JSON, Feldabgleich, Embeddings (`entity_embeddings`) und Observation-Tokens → Verifikation über Referenzbilder (`entity_file_links` + `asset_ai_prepass`-Captions) + Cosine-Similarity → Auto-Assign bei Score/Margin, sonst Review-Queue-Eintrag.
- Logische Entity-Type-Vorsortierung nach Beobachtung + SUBJECT_FIRST-Prepass: alle `entity_types` des Projekts werden anhand der Visual-Flags (Person/Tier/Objekt/Umgebung) und Priors gewichtet, offensichtliche Fehl-Passungen (z. B. Creature ohne Tiere im Bild, reine Umgebung) werden ausgeschlossen; die sortierte Kandidatenliste fließt in das Audit/JSON des KI-Runs ein.
- Auto-Assign verknüpft das Inventory mit dem Gewinner (`entity_file_links`, `asset_entities`/`primary_entity_id`), schreibt dynamische Achsenwerte in `inventory_classifications`/`revision_classifications` basierend auf `classification_axes` und leitet den `classification_state` daraus ab; Achsen werden nur bei hinreichender Confidence gefüllt, unsichere Fälle bleiben in `ai_review_queue`.
